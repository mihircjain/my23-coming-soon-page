      ...
      <main className="flex-1 px-6 md:px-12 pb-12">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="last7days" className="space-y-6">
            ...
            {/* Multi-line Chart Component for 7-day nutrition data with weekly averages */}
            <Card className="bg-white/60 backdrop-blur-sm border border-white/30 mb-6">
              <CardHeader>
                <CardTitle className="mobile-heading">7-Day Nutrition Trends</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="bg-white/60 backdrop-blur-sm border border-white/30 mb-6">
              <CardHeader>
                <CardTitle className="mobile-heading flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <span className="flex items-center">
                    <Calendar className="mr-2 h-5 w-5" />
                    {safeFormatDateForDisplay(selectedDate)}
                  </span>
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button
                          variant="outline"
                          className={cn(
                            "justify-start text-left font-normal touch-target text-xs sm:text-sm",
                            !selectedDate && "text-muted-foreground"
                          )}
                        >
                          <CalendarIcon className="mr-2 h-4 w-4" />
              ...
              <CardContent className="space-y-4">
                {/* Food Selector */}
                <FoodSelector onFoodAdd={handleAddFood} />
                ...
                <Card className="bg-gradient-to-r from-green-100 to-blue-100 border border-green-200">
                  <CardContent className="mobile-card">
                    <h3 className="font-semibold text-base sm:text-lg mb-3 text-center">Daily Totals</h3>
                    <div className="mobile-grid-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 text-center">
                      <div>
                        <div className="text-xl sm:text-2xl font-bold text-orange-600">{Math.round(totals.calories)}</div>
                        <div className="text-xs sm:text-sm text-gray-600">Calories</div>
                      </div>
                      <div>
                        <div className="text-xl sm:text-2xl font-bold text-green-600">{Math.round(totals.protein)}g</div>
                        <div className="text-xs sm:text-sm text-gray-600">Protein</div>
                      </div>
                      <div>
                        <div className="text-xl sm:text-2xl font-bold text-blue-600">{Math.round(totals.carbs)}g</div>
                        <div className="text-xs sm:text-sm text-gray-600">Carbs</div>
                      </div>
                      <div>
                        <div className="text-xl sm:text-2xl font-bold text-purple-600">{Math.round(totals.fat)}g</div>
                        <div className="text-xs sm:text-sm text-gray-600">Fat</div>
                      </div>
                      <div>
                        <div className="text-xl sm:text-2xl font-bold text-teal-600">{Math.round(totals.fiber)}g</div>
            ...
            <Card className="bg-white/60 backdrop-blur-sm border border-white/30 mb-6">
              <CardHeader>
                <CardTitle className="mobile-heading">Last 7 Days Overview</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="mobile-grid-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-4">
                  {last7DaysData.map((dayLog, index) => (
                    <DailyMacroBox
                      key={dayLog.date}
                      log={dayLog}
                      date={dayLog.date}
                      isToday={dayLog.date === safeGetTodayDateString()}
                      onClick={() => handleDateSelect(new Date(dayLog.date))}
                    />
                  ))}
                </div>
              </CardContent>
            ...
            {/* Meal Presets */}
            <Tabs defaultValue="daily-plans" className="w-full mb-6">
              <TabsList className="grid w-full grid-cols-1 sm:grid-cols-2 mb-6">
                <TabsTrigger value="daily-plans" className="text-sm">Daily Meal Plans</TabsTrigger>
                <TabsTrigger value="combined-meals" className="text-sm">Combined Meals</TabsTrigger>
              </TabsList>
              
              <TabsContent value="daily-plans">
                <Card className="bg-white/60 backdrop-blur-sm border border-white/30">
                  <CardHeader>
                    <CardTitle className="mobile-heading">Weekly Meal Plans</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="mobile-grid-2 lg:grid-cols-3 gap-4">
              ...
              <TabsContent value="combined-meals">
                <Card className="bg-white/60 backdrop-blur-sm border border-white/30">
                  <CardHeader>
                    <CardTitle className="mobile-heading">Combined Meal Presets</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="mobile-grid-2 lg:grid-cols-3 gap-4">
